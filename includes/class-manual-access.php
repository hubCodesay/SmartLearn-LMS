...
class SmartLearn_LMS_Manual_Access {
	...
	private static $active_access_cache = array();
	private static $course_user_ids_cache = array();
	private static $product_purchase_user_ids_cache = array();
	private static $history_table_exists = null;
	private static $course_product_map = null;
	...
	public static function run_one_time_maintenance() {
		global $wpdb;
	...
	public function process_expired_accesses() {
		...
		foreach ( (array) $history_rows as $history_row ) {
			$revoked_keys[ absint( $history_row->user_id ) . ':' . absint( $history_row->course_id ) . ':' . (string) $history_row->expires_at ] = true;
		}
		...
		foreach ( $rows as $r ) {
			...
			$revoke_key = $user_id . ':' . $course_id . ':' . $expires_at;

			if ( isset( $revoked_keys[ $revoke_key ] ) ) {
				continue;
			}
	...
	private static function invalidate_runtime_caches( $user_id = 0, $course_id = 0, $product_id = 0 ) {
		if ( $user_id && $course_id ) {
			unset( self::$active_access_cache[ $user_id . ':' . $course_id ] );
		}

		if ( $course_id ) {
			unset( self::$course_user_ids_cache[ $course_id . ':0' ] );
			unset( self::$course_user_ids_cache[ $course_id . ':1' ] );
		}

		if ( $product_id ) {
			unset( self::$product_purchase_user_ids_cache[ $product_id ] );
		}
	}

	private static function get_course_product_map() {
		if ( null !== self::$course_product_map ) {
			return self::$course_product_map;
		}
	...
	public static function grant_access( $user_id, $course_id, $expires_at = '', $note = '', $granted_by = 0 ) {
		...
		self::invalidate_runtime_caches( $user_id, $course_id, absint( get_post_meta( $course_id, '_smartlearn_course_product_id', true ) ) );
	...
	public function handle_order_completed( $order_arg ) {
		...
		foreach ( $order->get_items() as $item ) {
			...
			$product_map = self::get_course_product_map();
			$course_ids = isset( $product_map[ $product_id ] ) ? $product_map[ $product_id ] : array();
			$courses = array();
			foreach ( $course_ids as $course_id ) {
				$course = get_post( $course_id );
				if ( $course && in_array( $course->post_status, array( 'publish', 'draft' ), true ) ) {
					$courses[] = $course;
				}
			}
	...
	public static function get_access_history( $args = array() ) {
		...
		$table_name = self::get_history_table_name();
		if ( null === self::$history_table_exists ) {
			$exists = $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table_name ) );
			self::$history_table_exists = ( $exists === $table_name );
		}
	...
	public static function get_course_user_ids( $course_id, $include_free_all_users = true ) {
		...
		$cache_key = $course_id . ':' . ( $include_free_all_users ? '1' : '0' );
		if ( isset( self::$course_user_ids_cache[ $cache_key ] ) ) {
			return self::$course_user_ids_cache[ $cache_key ];
		}
		...
		self::$course_user_ids_cache[ $cache_key ] = $user_ids;
		return $user_ids;
	}
	...
	private static function get_user_ids_by_product_purchase( $product_id ) {
		...
		if ( isset( self::$product_purchase_user_ids_cache[ $product_id ] ) ) {
			return self::$product_purchase_user_ids_cache[ $product_id ];
		}
		...
		if ( empty( $rows ) ) {
			self::$product_purchase_user_ids_cache[ $product_id ] = array();
			return array();
		}

		self::$product_purchase_user_ids_cache[ $product_id ] = array_map( 'absint', $rows );
		return self::$product_purchase_user_ids_cache[ $product_id ];
	}
	...
	public static function user_has_active_access( $user_id, $course_id ) {
		...
		if ( isset( self::$active_access_cache[ $cache_key ] ) ) {
			return self::$active_access_cache[ $cache_key ];
		}
		...
		self::$active_access_cache[ $cache_key ] = ( $count > 0 );
		return self::$active_access_cache[ $cache_key ];
	}
	...
	public function handle_delete_access() {
		...
		$deleted = $wpdb->delete( $table_name, array( 'id' => $id ), array( '%d' ) );
			if ( $deleted && $row ) {
				self::invalidate_runtime_caches( absint( $row->user_id ), absint( $row->course_id ), absint( get_post_meta( absint( $row->course_id ), '_smartlearn_course_product_id', true ) ) );
		...
		if ( ! empty( $missing_ids ) ) {
			...
			if ( false !== $updated ) {
				foreach ( $course_user_ids as $target_user_id ) {
					self::invalidate_runtime_caches( $target_user_id, $course_id, absint( get_post_meta( $course_id, '_smartlearn_course_product_id', true ) ) );
