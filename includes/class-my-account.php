...
class SmartLearn_LMS_My_Account {
	...
	private $product_course_map = null;
	...
	private function get_user_courses( $user_id ) {
		...
		foreach ( $all_course_ids as $course_id ) {
			...
			$is_active = false;
			if ( $has_course_post ) {
				$is_active = ! empty( $manual ) || ! empty( $purchased_courses[ $course_id ] );
				if ( ! $is_active ) {
	...
	private function get_product_course_map() {
		if ( null !== $this->product_course_map ) {
			return $this->product_course_map;
		}
