...
define( 'SMARTLEARN_LMS_VERSION', '1.0.4' );
...
class SmartLearn_Courses_LMS {
	...
	private function init_hooks() {
		...
		add_action( 'plugins_loaded', array( $this, 'maybe_run_updates' ), 20 );
	...
	public function maybe_run_updates() {
		$stored_version = (string) get_option( 'smartlearn_lms_version', '' );
		if ( SMARTLEARN_LMS_VERSION === $stored_version ) {
			return;
		}

		SmartLearn_LMS_Manual_Access::create_table();
		SmartLearn_LMS_Manual_Access::create_history_table();
		SmartLearn_LMS_Manual_Access::run_one_time_maintenance();

		if ( class_exists( 'SmartLearn_LMS_Notifications' ) ) {
			SmartLearn_LMS_Notifications::create_table();
		}

		update_option( 'smartlearn_lms_version', SMARTLEARN_LMS_VERSION );
	}
